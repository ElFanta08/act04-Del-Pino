/*
Nom: Jordi Torrella Lopez, Joan Del Pino Molina
Echo en equipo :)
...
void comprobarcarta(carta mano[], int contadorAs, int *puntos, int *num_cartas_mano, bool *repetir, int Carta);
int main()
{
  int Carta;
    ...
    if (turno == 0) // en el caso de que turno sea igual a 0...
    {
      ...
      for (int Carta = 0; Carta < 52; Carta++)
      {
        if (mano[Carta].palo != "")
        {
            ...
            comprobarcarta(mano, contadorAs, &puntos, &num_cartas_mano, &repetir, Carta);
        }
        }
      }
      ...
      case 1:
        ...
        mano[Carta + 1] = baraja[rand() % 52]; // te saca carta
...
void comprobarcarta(carta mano[], int contadorAs, int *puntos, int *num_cartas_mano, bool *repetir, int Carta)
{
  ...
  if (mano[Carta+ 1].numero == 1)
  { // lo del AS
    cout << "\033[1;32m A de " << mano[Carta + 1].palo << "\033[0m" << endl;
    contadorAs++;
  ...
  else if (mano[Carta].numero == 11)
  {                                                                         // si el numero es 11
    cout << "\033[1;33m Sota de " << mano[Carta].palo << "\033[0m" << endl; // cout de sota
    *puntos = *puntos + 10;                                                 // representa +10 de valor de la jugada
    *num_cartas_mano++;                                                     // aumentamos 1 carta a la mano
  }
  else if (mano[Carta].numero == 12)
  { // si es el 12... ( caballo)
    cout << "\033[1;33m Caballo de " << mano[Carta].palo << "\033[0m" << endl;
    *puntos = *puntos + 10;
    *num_cartas_mano++; // aumentamos 1 carta a la mano
  }
  else if (mano[Carta].numero == 13)
  { // si es el 13... ( caballo)
    cout << "\033[1;33m Rey de " << mano[Carta].palo << "\033[0m" << endl;
    *puntos = *puntos + 10;
    *num_cartas_mano++; // aumentamos 1 carta a la mano
  }
  else
  { // TODAS LAS DEMAS CARTAS QUE NO SEAN FIGURAS...
    cout << "\033[1;33m " << mano[Carta].numero << " de " << mano[Carta].palo << "\033[0m" << endl;
    *puntos = *puntos + mano[Carta].numero;
    *num_cartas_mano++; // aumentamos 1 carta a la mano
  }
}